 ...
 * @param {Number} numRecords the number of records to be created for each entity
...
const asJson = module.exports = async (data, csn, numRecords=1, options={}) => {
  ...
  Object.entries(referenceData).filter(([, name]) => entityNames.includes(name)).forEach(([file]) => delete referenceData[file])
...
function fillRecord(record, recordID, entity, csn, data, parentEntity, depth, referenceData) {
  Object.defineProperty(record, '$id', {value: recordID, enumerable: false})

  const { elements } = entity
  Object.values(elements).sort(asJson.elementsSorter(csn)).forEach(el => {
    record[el.name] = dataForField(el, entity, csn, record, data, parentEntity, depth, referenceData)
  })
  return record
}
...
function dataForField(element, entity, csn, record, data, parentEntity, depth, referenceData) {
  ...
  if (element.items?.elements)  // `array of` / `many` with complex type
    ...
    return [ dataForField(element.items, entity, csn, record, data, null, depth+1, referenceData) ]
  ...
  switch (type) {
    ...
    case 'cds.String': {
      ...
      if (element.enum) {
        ...
        if (element.key && entity.elements.name && entity.elements.descr)
      ...
      if (element.key) {
        ...
        if (element.name === 'locale' && entity.name.endsWith('texts')) {
          const record = randomFromReferenceData(entity, csn, referenceData)
    ...
    case 'cds.Composition': {
      ...
      for (let i = 0; i < targetRecordCount; i++) {
        targetRecords.push(fillRecord({}, targetRecordID+i, target, csn, data, entity, depth+1, referenceData))
      }
    ...
    case 'cds.Vector': // TODO add Vector
      return
    default: {
      console.error(`Unknown type ${element._type} (${element.type}) in '${entity.name}:${element.name}'`)
      return
    }
  }
}
...
function uniqRecords(records, entity) {
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (keySet.has(key)) {
      // console.debug('Removing', keys, key, 'from', entity.name)
      records.splice(i, 1)
      i--
    }
    keySet.add(key)
