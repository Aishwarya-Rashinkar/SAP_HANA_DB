...
class BindingResolver {
    ...
    async resolveBindings({ bindings, cwd = process.cwd(), env, cache = {} }) {
        ...
        await Promise.all(Object.keys(bindings).map(async name => {
            ...
            if (!cache[cacheKey]) {
                ...
                logger.log(`bound ${bold(name)} to ${platformText(binding)} ${bindingText(binding)}`)
            } else {
    ...
    resolveBinding(name, binding) {
        switch (binding.type) {
            ...
            default: throw new Error(`Unsupported binding type ${bold(binding.type)} for service ${bold(name)}.`)
        }
    }
}
